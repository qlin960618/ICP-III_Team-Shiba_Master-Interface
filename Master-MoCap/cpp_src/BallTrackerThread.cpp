...
int main(int argc, char **argv)
{
    ...
    try
    {
        ...
        while(true)
        {
            ...
            #ifdef USE_RGB
            cv::GaussianBlur(frame, frame_pre_thresh, cv::Size(11, 11), 0);
            #else
            cv::GaussianBlur(frame, frame_blured, cv::Size(11, 11), 0);
            cv::cvtColor(frame_blured, frame_pre_thresh, cv::COLOR_BGR2HSV);
            #endif

            cv::inRange(frame_pre_thresh, ball_0_low, ball_0_high, mask_0);
            cv::inRange(frame_pre_thresh, ball_1_low, ball_1_high, mask_1);
